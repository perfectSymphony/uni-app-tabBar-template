<template>
	<div>
		<div v-show="isShowList" class="pop-mask"></div>
		<div class="show-box" :class="{disabled: disabled, active: isShowList}" :style="style_Container">
			<!-- 输入框，仅在可输入模式下使用 -->
			<input  
				v-if="showInput"  class="input"  placeholder-style="color: #f00;"
				type="text" v-model="selectText"  :placeholder="placeholder"
				@focus="onFocus" @blur="onBlur" @input="onInput" @confirm="$emit('confirm', $event)" @click="onUpperClick"
			>
			<!-- 显示框 -->
...
<script>
	import Vue from 'vue';
	Vue.__lzw_select = Vue.__lzw_select || new Vue();  // 这个实例专门用来做lzw-select多个实例之间的通信中间站
	export default {
		name: 'lzw-select',
		props: {
			list: {            // 原始数据
		...
		mounted(){
			Vue.__lzw_select.$on('open', this.onOtherLzwSelectOpen);
		...
		beforeDestroy(){
			Vue.__lzw_select.$off('open', this.onOtherLzwSelectOpen);
		},
		methods: {
			onOtherLzwSelectOpen(component){ //当本组件的其它实例展开时的回调
			...
			onListShow(){  //列表显示时的回调
				...
				Vue.__lzw_select.$emit('open', this);
			}
			/************************** 列表的操作(显示/隐藏/点击) ****************************/
		}
	}
	...
	.pop-mask {
		...
		.show-box{
			...
			.right-arrow{
				...
				position: absolute; 
				font-size: 1em; right: 0px; display: flex; 
				top: 0;
				align-items: center; color: $hover-color; height: 100%;
				font-weight: 100; width: 0; justify-content: center;
			}
			.clear{
